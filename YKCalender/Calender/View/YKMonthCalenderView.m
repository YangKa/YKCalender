...
#import "YKMonthCalenderView.h"

@interface YKMonthCalenderView ()<UITableViewDelegate, UITableViewDataSource>{
...
@implementation YKMonthCalenderView
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 0) {
        ...
        cell.textLabel.text = [NSString stringWithFormat:@"%ld月", indexPath.row+1];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView.tag == 0 && indexPath.row != _curSelectSection) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectSignleCalender:calenderType:)]) {
            ...
            [self.delegate didSelectSignleCalender:model calenderType:YKCalenderType_month];
        }
    }
...
- (YKCalenderModel*)calenderModelForIndex:(NSIndexPath*)indexPath{
    ...
    NSUInteger curMonth = indexPath.row+1;
    
    return [[YKCalenderModel alloc] initWithYear:curYear month:curMonth];
}
